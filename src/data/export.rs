...
impl DataExporter {
    ...
    pub fn export_markdown(&self, measurements: &[TemperatureMeasurement]) -> IoResult<()> {
        ...
        writeln!(
            file,
            "| X (mm) | Y (mm) | Angle (d:m:s) | T_min (°C) | T_max (°C) |"
        )?;
        writeln!(
            file,
            "|--------|--------|---------------|------------|------------|"
        ...
        for meas in measurements {
            writeln!(
                file,
                "| {:.2} | {:.2} | {} | {:.2} | {:.2} |",
                meas.x_mm, meas.y_mm, meas.angle_dms, meas.temp_min, meas.temp_max
            )?;
        }
    ...
    pub fn export_csv(&self, measurements: &[TemperatureMeasurement]) -> IoResult<()> {
        ...
        for meas in measurements {
            writeln!(
                file,
                "{:.2},{:.2},{},{:.2},{:.2}",
                meas.x_mm, meas.y_mm, meas.angle_dms, meas.temp_max, meas.temp_min
            )?;
        }
...
mod tests {
    ...
    fn test_export_markdown() {
        ...
        let measurements = vec![TemperatureMeasurement {
            x_mm: 10.0,
            y_mm: 20.0,
